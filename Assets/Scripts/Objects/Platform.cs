...
/// <summary>
/// Controls the behavior and appearance of platform objects in the game.
/// </summary>
[RequireComponent(typeof(SpriteRenderer))]
public class Platform : MonoBehaviour    // *** TO DO *** : Broke into multiple classes
{
    [Header("Movement Variables")] public int 
        movement, 
        rotationAngle, 
        rotationSpeed;
    ...
    private void Start()
    {
        if (platformType != PlatformType.CircularMoving)
        {
            ...
            Instantiate(ghostBlock, currentPosition, Quaternion.identity).GetComponent<GhostBlock>().originPlatform =
                this;
            var ghostBlockObject = Instantiate(ghostBlock, currentPosition, Quaternion.identity, transform)
                .GetComponent<GhostBlock>();
        ...
        switch (randomNumber)
        {
            ...
            case < 10: //Spike
                if (platformType != PlatformType.Temporary)
                    Instantiate(spike, selfPosition + Vector3.up * .75f + Vector3.back, Quaternion.identity,
                        selfTransform);
                return;
    ...
    private void CreatePlant()
    {
        ...
        plant.transform.localPosition = new Vector3((float)(random.NextDouble() - .5f),
            (float)(random.NextDouble() / 4) + (randomPlant > 1 ? .45f : 1.5f), -1);
