...
public class MenuLight : MonoBehaviour
{
    ...
    private void SetUpTheme()
    {
        ...
        localTransform.parent.parent.GetComponent<Image>().color = _darkMode ? new Color(0.1f, 0.1f, 0.3f) : Color.cyan;
        localTransform.parent.GetComponent<Image>().color =
            _darkMode ? new Color(0.5f, 1f, 0.8f, 0.5f) : new Color(0f, 1f, 0.5f, 1f);

        var selfImage = localTransform.GetComponent<Image>();

        selfImage.color = _darkMode ? new Color(0.4f, 0.4f, 1) : Color.white;
        selfImage.sprite = _darkMode ? darkSun : lightSun;

        localTransform.GetChild(0).gameObject.SetActive(_darkMode);
    }
}